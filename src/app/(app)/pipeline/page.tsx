...
const STAGES: { key: PipelineStage; label: string; color: string; dot: string; tab: string }[] = [
  { key: 'lead',        label: 'Lead',       color: 'bg-gray-50  border-gray-200',   dot: 'bg-gray-400',   tab: 'text-gray-600'   },
  { key: 'contactado',  label: 'Contactado', color: 'bg-blue-50  border-blue-200',   dot: 'bg-blue-500',   tab: 'text-blue-600'   },
  { key: 'proposta',    label: 'Proposta',   color: 'bg-yellow-50 border-yellow-200', dot: 'bg-yellow-500', tab: 'text-yellow-600' },
  { key: 'negociacao',  label: 'Negociação', color: 'bg-orange-50 border-orange-200', dot: 'bg-orange-500', tab: 'text-orange-600' },
  { key: 'fecho',       label: 'Fecho',      color: 'bg-green-50 border-green-200',  dot: 'bg-green-500',  tab: 'text-green-600'  },
  { key: 'perdido',     label: 'Perdido',    color: 'bg-red-50   border-red-200',    dot: 'bg-red-400',    tab: 'text-red-500'    },
]
...
export default function PipelinePage() {
  ...
  return (
    <div>
          ...
          return (
            <button
              ...
              className={`rounded-xl border-2 p-3 text-left transition-all ${
                active
                  ? `${stage.color} border-opacity-100 shadow-md scale-[1.02]`
            ...
            >
              ...
              {val > 0 && <p className="text-xs text-gray-500 truncate">{formatCurrency(val)}</p>}
            </button>
          )
        })}
      </div>
      ...
      <div className={`rounded-2xl border-2 ${activeConfig.color} p-4`}>
        <div className="flex items-center justify-between mb-4">
          <div className="flex items-center gap-2">
            <span className={`w-3 h-3 rounded-full ${activeConfig.dot}`} />
            <h2 className="font-semibold text-gray-800">{activeConfig.label}</h2>
            <span className="text-sm text-gray-500">({activeItems.length})</span>
          </div>
          <div className="flex items-center gap-1">
            <button
              onClick={() => activeIdx > 0 && setActiveStage(STAGES[activeIdx - 1].key)}
              disabled={activeIdx === 0}
              className="p-1.5 rounded-lg text-gray-400 hover:text-gray-600 hover:bg-white/60 disabled:opacity-30"
            >
              <ChevronLeft size={16} />
            </button>
            <button
        ...
        {loading ? (
          ...
          <div className="text-center py-10">
            ...
            <p className="text-gray-400 text-sm">Nenhuma oportunidade em {activeConfig.label}</p>
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex flex-wrap items-center gap-x-3 gap-y-0.5 mt-1">
                      {opp.client && <span className="text-xs text-gray-500">{opp.client.name}</span>}
                      {opp.expected_close_date && (
                        <span className="text-xs text-gray-400">Fecho: {formatDate(opp.expected_close_date)}</span>
                      )}
                      {opp.owner && <span className="text-xs text-gray-400">{opp.owner.full_name}</span>}
                    </div>
                  </div>

                  {/* Valor */}
                  <p className="text-lg font-bold text-gray-900 shrink-0">{formatCurrency(opp.value)}</p>
